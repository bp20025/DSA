...
typedef struct AdjListNode { // 隣接リストのノード
    int dest;
    int weight;
    struct AdjListNode* next;
} AdjListNode;

typedef struct AdjList { // 隣接リスト
    AdjListNode* head;
} AdjList;
...
AdjListNode* newAdjListNode(int dest, int weight) { // 新しい隣接リストのノード
...
void addEdge(Graph* graph, int src, int dest, int weight) { // グラフにeを追加
