...
void heapSort(int a[], int hi); // ヒープソートを実行する関数
void pushdown(int a[], int k, int hi); // 子ノードとの比較
void swap(int *a, int *b); // swap関数
void printArray(int a[], int lo, int hi ); // 配列の一覧表示

void insertNode(int a[], int val); // ヒープへのノードの挿入
void pushup (int a[], int k); // 親ノードとの比較

int main() {
  int X[MAX_SIZE]; // ヒープを表現する配列
  ...
  // 更に要素を追加
  printf("更にノードを追加します: 4\n");
  insertNode(X, 4);
  printArray(X, 0, size - 1);
  
  printf("更にノードを追加します: 1\n");
  insertNode(X, 1);
  printArray(X, 0, size - 1);
...
void heapSort (int a[], int hi) { // （1~ヒープソートを実行する関数
  // ヒープを構成する部分／子を持つノードに関して「親＜子」関係を担保
...
//   insertNode(X, 10);
void insertNode(int a[], int val) { // ヒープへノードを挿入する関数
  // ヒープ末尾へノードを挿入
  a[size] = val;
  size++;

  pushup(a, size-1); // 親ノードとの比較
}
