...
// ノードを移動する関数を10000回実行し、平均移動回数を導出する
...
int main( int argc, char *argv[] ) {
  ...
  int aveMoveCounter = 0;

  // 「1-3-1」での処理を10000回繰り返す
  for (i = seed; i < seed + 10000; i++) {
    // シード値を画面表示する
    printf("seed: %d\n", i);

    // ノードの移動の変遷を画面表示する
    nodeMovingJudge(adjacent, i);

    // ノードを移動した回数を画面表示する
    printf("移動回数: %d\n", moveCounter);

    aveMoveCounter += moveCounter;
