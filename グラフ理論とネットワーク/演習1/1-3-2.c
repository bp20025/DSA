...
// 1-3-1 を10,000回繰り返し，平均移動回数を算出する
...
// グラフをランダムに移動
int moveRandom(int seed, int N, int **adjacent);
...
int main( void ) {
  ...
  // 作成した配列（隣接行列）より，点をランダムに移動する
  int seed;
  int ahah; // 経路数の一時変数
  int aveNumOfPaths = 0; // 経路数（ave）
  int trials = 10000; // 試行回数

  printf("Move randomly across the graph\n");

  printf("Number of trials: %d\n", trials);
  
  
  for (seed = 1; seed <= trials; seed++) {
    ahah = moveRandom(seed, N, adjacent);
    // printf("Num of Paths: %d\n", ahah); // 経路数の結果
    aveNumOfPaths += ahah;
  }
  aveNumOfPaths /= trials;

  printf("Average number of paths: %d\n", aveNumOfPaths);
...
int moveRandom(int seed, int N, int **adjacent) {
  ...
  srand48((float)seed); // 乱数の初期化

  int pos = 0; // 現在の点
  int next; // 次の点
  int countPath = 0; // 経路数のカウンタ

  // printf("The path traveled randomly（seed: %d）\n", seed);
  // printf("%d", pos);

  while (pos != 6) {
    int r;
    do {
      r = (int)(drand48() * N); // 0からN-1までのランダムな整数を生成（next候補）
      next = r;
    } while (adjacent[pos][next] == 0); // next候補が隣接していない場合は再試行

    // printf(" -> %d", next); // 移動先を表示
    countPath++; // 経路数の更新
    pos = next; // 現在の点の更新
  }
  // printf("\n");
